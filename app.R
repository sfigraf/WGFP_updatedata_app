...
# PlotOutputs -------------------------------------------------------------
    ...
    output$plot1 <- renderPlotly({
      if(is.null(plot_ready_new_data())){
        plot_ly() %>%
          layout(
            title = "Empty Plot: No New Detections Uploaded",
        ...
        plot_ready_new_data() %>%
          plot_ly(
            x = ~hour1, 
            color = ~SCD, 
            text = ~SCD, # Pass the site code to the text variable
            type = "histogram",
            # %{y} grabs the count, %{x} grabs the hour, %{text} grabs the site code
            # <extra></extra> hides the annoying secondary box Plotly usually adds!
    ...
    output$markerTagPlot <- renderPlotly({
      ...
      plot_ready_new_data() %>%
        plot_ly(
          ...
          # Build the exact string you want using HTML breaks (<br>)
          text = ~paste("Scan Date:", Scan_Date, "<br>Scan Time:", Scan_Time, "<br>TAG:", TAG),
          hoverinfo = "text", # Tell Plotly to ONLY show our custom text string
          type = "scatter", 
          mode = "markers"
        ) %>%
        layout(
